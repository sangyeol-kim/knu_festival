...
namespace :crawlling_weather do
  ...
  task crawlling_weather: :environment do
    ...
    headers = { :params => { CGI::escape('numOfRows') => '80',CGI::escape('pageNo') => '1', CGI::escape('base_date') => "#{(Time.now+9.hours).strftime('%Y%m%d')}", CGI::escape('base_time') => "0500", CGI::escape('nx') => '73', CGI::escape('ny') => '133' } }
